...
def load_model(trial: optuna.Trial, *, window_size: float):
    channels = [c for c in [
    ...
    n_harmonics = trial.suggest_int('n_harmonics', 2, 5)
    ...
    model = CACC(filter_banks=filter_banks,
                n_harmonics=n_harmonics, a=a, b=b,
                preprocess=preprocess)
    return model


def objective(trial: optuna.Trial, *, eegs_train, eegs_test, window_size: float, n_preds: int, preds_per_sec: int):
    ...
    return np.mean([model.test(eeg) for eeg in eegs_test])
...
@click.option('--subject', default='unknown', show_default=True)
@click.option('--study', default='{timestamp}-{subject}-{window_size}', show_default=True)
...
def main(*,
    ...
    study = optuna.create_study(
        study_name=study.format(window_size=window_size, n_preds=n_preds, preds_per_sec=preds_per_sec, subject=subject,
                                timestamp=timestamp),
        storage=db,
        directions=['maximize'],
    ...
    optimize(
        func=partial(objective, eegs_train=eegs_train, window_size=window_size, eegs_test=eegs_test, n_preds=n_preds, preds_per_sec=preds_per_sec),
        n_trials=n_trials,
