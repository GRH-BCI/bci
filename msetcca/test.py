...
def test(eegs_train, eegs_test, *, window_size: float, n_preds: int, preds_per_sec: int):
    ...
    model = MsetCCA(preprocess=preprocess)
    ...
    #         model.train_add(eeg[:, 300:300+int(3.5*300)])
    #     model.train_finish()
...
def main(*,
    ...
    with timer('Loading'):
        ...
        eegs_train = [chunkify(eeg, window_size=window_size, stride=10/300) for eeg in eegs_test]
